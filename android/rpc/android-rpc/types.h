...
struct xdr_ops_struct {
  ...
  void        (*xdr_destroy) (xdr_s_type *xdr);					//0x00
  bool_t      (*xdr_control) (xdr_s_type *xdr, int request, void *info);	//0x04

  /* Incoming message control functions */
  bool_t (*read)           (xdr_s_type *xdr);					//0x08
  bool_t (*msg_done)       (xdr_s_type *xdr);					//0x0c

  /* Outgoing message control functions */
  bool_t (*msg_start) (xdr_s_type *xdr, rpc_msg_e_type rpc_msg_type);		//0x10
  bool_t (*msg_abort) (xdr_s_type *xdr);					//0x14
  bool_t (*msg_send)  (xdr_s_type *xdr);					//0x18
  
  /* Message data functions */
  bool_t (*send_int8)   (xdr_s_type *xdr, const int8 *value);			//0x1c
  bool_t (*send_uint8)  (xdr_s_type *xdr, const uint8 *value);			//0x20
  bool_t (*send_int16)  (xdr_s_type *xdr, const int16 *value);			//0x24
  bool_t (*send_uint16) (xdr_s_type *xdr, const uint16 *value);			//0x28
  bool_t (*send_int32)  (xdr_s_type *xdr, const int32 *value);			//0x2c
  bool_t (*send_uint32) (xdr_s_type *xdr, const uint32 *value);			//0x30
  bool_t (*send_bytes)  (xdr_s_type *xdr, const uint8 *buf, uint32 len);	//0x34

  bool_t (*recv_int8)   (xdr_s_type *xdr, int8 *value);				//0x38
  bool_t (*recv_uint8)  (xdr_s_type *xdr, uint8 *value);			//0x3c
  bool_t (*recv_int16)  (xdr_s_type *xdr, int16 *value);			//0x40
  bool_t (*recv_uint16) (xdr_s_type *xdr, uint16 *value);			//0x44
  bool_t (*recv_int32)  (xdr_s_type *xdr, int32 *value);			//0x48
  bool_t (*recv_uint32) (xdr_s_type *xdr, uint32 *value);			//0x4c
  bool_t (*recv_bytes)  (xdr_s_type *xdr, uint8 *buf, uint32 len);		//0x50
};
